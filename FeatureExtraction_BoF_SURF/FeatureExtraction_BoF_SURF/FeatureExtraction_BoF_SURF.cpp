...
// 1. Building a visual vocabulary from a subset of the training images by extracting SURF feature points and computing the descriptor for each point, followed by K-Means clustering
// 2. Extracting SURF feature points and computing descriptors for all the images in the training set, matching the descriptors with the vocabulary defined earlier and extracting a histogram
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	//Initialising the Hessian Threshold
	int hessianThreshold = atoi(applicationConfig.FirstChildElement("APPCONFIG")->FirstChildElement("HessianThreshold")->GetText());

	//Initialising the Number of Octaves to be used during SURF feature detection and description
	int numberOfOctaves = atoi(applicationConfig.FirstChildElement("APPCONFIG")->FirstChildElement("NumberOfOctaves")->GetText());

	//Initialising the Number of Octave Layers to be used during SURF feature detection and description
	int octaveLayers = atoi(applicationConfig.FirstChildElement("APPCONFIG")->FirstChildElement("OctaveLayers")->GetText());
	...
	//A vector that will be populated with SURF feature points extracted from the current image
	vector<KeyPoint> featurePoints;

	//A matrix to hold the SURF feature descriptor of the current image
	Mat featureDescriptor;

	//A matrix to hold all the SURF feature descriptors from all the images in the training set subset
	Mat combinedFeatureDescriptors;

	//Initialising the SURF feature detector and descriptor
	SurfDescriptorExtractor descriptorVector(hessianThreshold, numberOfOctaves, octaveLayers, false, true);
	...
	for (int i = 1; i <= vocabularyImages; i++){
		...
		if (!currentImage.empty())
		{
			//Get the SURF feature points from the current image
			descriptorVector.detect(currentImage, featurePoints);

			//Get the SURF feature descriptor for the extracted feature points
			descriptorVector.compute(currentImage, featurePoints, featureDescriptor);
	...
	//Initialise the SURF feature detector
	Ptr<FeatureDetector> featureDetector(new SurfFeatureDetector(hessianThreshold, numberOfOctaves, octaveLayers, false, true));

	//Initialise the SURF feature descriptor
	Ptr<DescriptorExtractor> featureDescriptor(new SurfDescriptorExtractor(hessianThreshold, numberOfOctaves, octaveLayers, false, true));
	...
	//const char * conceptList = applicationConfig.FirstChildElement("APPCONFIG")->FirstChildElement("StandardConcepts")->GetText();
